...
Title: TTS MERGPARS - Dash Web Application for Merging Temperature Time Series

Description:
    Modern web-based interface to process, merge, and export temperature logger data.
    Replaces legacy CLI version (tts_mergpars.py) with interactive Dash front-end.

Features:
    - Upload CSVs for shallow/deep logger data
    - Full water year conversion and merging
    - Interactive time-series visualization
    - Export selected intervals (CSV, WYO)
    - Default parameter loading from `.par` file
    - Session-based logging to per-run log files
    - Dedicated log directory + automated management

Author: Timothy Wu (converted from legacy version)  
Created: 2025-07-08  
        ...
        dbc.CardBody([
            ...
            dbc.Row([
                dbc.Col([
                    ...
                    html.Div(id="shallow-filename-display", className="mt-1 text-muted small"),
                ...
                dbc.Col([
                    ...
                    html.Div(id="deep-filename-display", className="mt-1 text-muted small"),
...
@app.callback(
    [Output('shallow-file-status', 'children'),
     ...
     Output('shallow-filename-display', 'children')],
...
def process_shallow_file(contents, filename, convert_temp, interval, gap_factor):
    ...
    try:
        ...
        filename_display = f"📄 Uploaded: {filename}"
        
        return status, df.to_json(date_format='iso', orient='split'), filename_display
        ...
        filename_display = f"❌ Error in: {filename}" if filename else "❌ Upload error"
        return status, None, filename_display

@app.callback(
    [Output('deep-file-status', 'children'),
     Output('deep-data-store', 'data'),
     Output('deep-filename-display', 'children')],
...
@app.callback(
    [Output('shallow-file-status', 'children', allow_duplicate=True),
     ...
     Output('shallow-filename-display', 'children', allow_duplicate=True)],
...
def load_shallow_from_par(n_clicks, stored_params, convert_temp, interval, gap_factor):
    ...
    try:
        ...
        if not os.path.exists(file_path):
            ...
            return status, None, f"❌ Not found: {shallow_filename}"
        ...
        filename_display = f"📄 Loaded from .par: {shallow_filename}"
...
def load_deep_from_par(n_clicks, stored_params, convert_temp, interval, gap_factor):
    ...
    try:
        ...
        if not os.path.exists(file_path):
            ...
            return status, None, f"❌ Not found: {deep_filename}"
        ...
        filename_display = f"📄 Loaded from .par: {deep_filename}"
